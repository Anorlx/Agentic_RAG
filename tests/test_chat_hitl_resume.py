...
class ChatHitlResumeTests(unittest.IsolatedAsyncioTestCase):
    async def test_stream_uses_supervisor_and_persists_combined_trace(self):
        fake_storage = FakeStorage()
        supervisor = Mock(return_value={
            "answer": "最新公开信息见 [W1]。",
            "rag_trace": {
        ...
        with (
            ...
            patch.object(service, "run_supervisor", supervisor),
            patch.object(service, "generate_session_title", Mock(return_value="水稻政策")),
        ):
            chunks = await _collect_stream("最新水稻政策", "u", "s")

        events = _parse_sse_events(chunks)
        self.assertEqual(["最新公开信息见 [W1]。"], [
            event["content"] for event in events if event.get("type") == "content"
        ])
        self.assertEqual("web_search", fake_storage.saves[-1]["extra_message_data"][-1]["rag_trace"]["supervisor_route"])
    ...
    async def test_stream_immediately_reports_progress_and_skips_note_for_short_chat(self):
        ...
        supervisor = Mock(return_value={
            "answer": "直接回答",
            "rag_trace": {"supervisor_route": "direct"},
            "rag_result": {},
        })
    ...
    async def test_stream_hitl_request_persists_pending_state_without_content(self):
        ...
        supervisor = Mock(return_value={
            "answer": "",
            "rag_trace": trace,
            "rag_result": {"hitl_resume_state": resume_state},
        })
    ...
    async def test_stream_resume_uses_saved_rag_state_without_reentering_agent(self):
        ...
        supervisor_mock = Mock(side_effect=AssertionError("supervisor should not run on HITL resume"))

        with (
            patch.object(service, "storage", fake_storage),
            patch.object(service, "run_supervisor", supervisor_mock),
