...
class RagLatencyGuardTests(unittest.TestCase):
    def test_local_reranker_loads_from_disk_without_network(self):
        cross_encoder = Mock()
        fake_sentence_transformers = types.ModuleType("sentence_transformers")
        fake_sentence_transformers.CrossEncoder = cross_encoder
        ...
        with patch.dict(sys.modules, {"sentence_transformers": fake_sentence_transformers}):
            ...
            loader = getattr(utils, "_get_local_reranker", None)
            self.assertIsNotNone(loader)
            self.assertIs(cross_encoder.return_value, loader())
    ...
    def test_local_reranker_orders_documents_without_http_api(self):
        utils, _ = load_utils({
            "LOCAL_RERANK_MODEL": "/models/bge-reranker-v2-m3",
            "RERANK_MODEL": "",
            "RERANK_BINDING_HOST": "",
        ...
        self.assertTrue(utils.RERANK_ENABLED)

        reranker = Mock()
        reranker.predict.return_value = [0.1, 0.9]
        with patch.object(utils, "_get_local_reranker", return_value=reranker):
            docs, meta = utils._rerank_documents(
                "query",
                [
                    {"text": "weaker", "chunk_id": "chunk-1", "score": 0.9},
                    {"text": "stronger", "chunk_id": "chunk-2", "score": 0.8},
                ],
                2,
            )

        reranker.predict.assert_called_once_with(
            [("query", "weaker"), ("query", "stronger")],
        )
        self.assertFalse(hasattr(utils, "requests"))
        self.assertTrue(meta["rerank_enabled"])
        self.assertTrue(meta["rerank_applied"])
        self.assertEqual("chunk-2", docs[0]["chunk_id"])
        self.assertEqual(0.9, docs[0]["rerank_score"])
