<template>
  <div v-if="msg.ragTrace" class="message-meta">
    <details class="reasoning-details">
      <summary>{{ showRetrievalDetails ? '检索过程' : '回复路径' }}</summary>
      <div class="reasoning-content">
        <div v-if="msg.ragTrace.supervisor_route" class="trace-line trace-route">
          回复路径：{{ formatSupervisorRoute(msg.ragTrace.supervisor_route) }}
        </div>
        <div v-if="msg.ragTrace.supervisor_reason" class="trace-line">
          选择理由：{{ msg.ragTrace.supervisor_reason }}
        </div>
        <div v-if="msg.ragTrace.web_search_status" class="trace-line">
          网页搜索：{{ formatWebSearchStatus(msg.ragTrace.web_search_status) }}
        </div>
        <template v-if="showRetrievalDetails">
...
const showRetrievalDetails = computed(() => shouldShowRetrievalDetails(
  props.msg.ragTrace?.supervisor_route,
));
