<template>
  <div class="settings-panel">
    <header class="settings-header">
      <div>
        <span class="panel-eyebrow">SuperAgenticRAG 知识库</span>
        <h1>知识库</h1>
        <p>管理 SuperAgenticRAG 可以检索的文档、索引与数据源。</p>
      </div>
      <button
        type="button"
    ...
    <div class="settings-grid">
      <section class="documents-section">
        <div class="documents-section-head">
          <div>
            <h2>全部文档</h2>
            <p>{{ filteredDocuments.length }} 份资料可供 SuperAgenticRAG 检索</p>
          </div>
        ...
        <div v-else-if="filteredDocuments.length === 0" class="empty-documents">
          ...
          <p>{{ searchQuery ? '换一个关键词试试。' : '从右侧上传第一份资料，让 SuperAgenticRAG 开始学习。' }}</p>
        </div>
