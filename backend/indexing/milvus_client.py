...
class MilvusStore:
    ...
    def get_document_leaf_chunks(self, filename: str) -> list[dict]:
        """Read L3 chunks for a document without embedding or retrieval."""
        safe_filename = filename.replace("\\", "\\\\").replace('"', '\\"')
        return self.query_all(
            filter_expr=f'filename == "{safe_filename}" and chunk_level == 3',
            output_fields=[
                "text",
                "filename",
                "file_type",
