...
async def get_document_chunks(filename: str, _: User = Depends(require_admin)):
    """Return existing L1/L2 parent chunks and L3 Milvus leaves for inspection."""
    try:
        parent_chunks = parent_chunk_store.list_by_filename(filename)
        leaf_chunks = milvus_manager.get_document_leaf_chunks(filename)
        return DocumentChunkListResponse(
            filename=filename,
            chunks=merge_document_chunks(parent_chunks, leaf_chunks),
        )
