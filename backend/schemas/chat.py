...
class RagTraceFields(StrictSchema):
    supervisor_route: Optional[Literal[
        "direct",
        "knowledge_base",
        "web_search",
    ...
    web_search_status: Optional[Literal["completed", "failed", "not_requested"]] = None
...
def _normalize_web_sources(value) -> list[dict]:
    if not isinstance(value, list):
        return []
    fields = WebSource.model_fields
    normalized_sources = []
    for item in value:
        if not isinstance(item, dict):
            continue
        candidate = {key: item[key] for key in fields if key in item}
        try:
            normalized_sources.append(
                WebSource.model_validate(candidate).model_dump(exclude_none=True)
            )
        except ValueError:
            continue
...
def _normalize_trace_fields(trace: dict, fields: dict) -> dict:
    ...
    if "web_sources" in normalized:
        normalized["web_sources"] = _normalize_web_sources(normalized["web_sources"])
    return normalized
