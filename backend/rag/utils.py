...
LOCAL_RERANK_MODEL = _optional_env("LOCAL_RERANK_MODEL")
LOCAL_RERANK_DEVICE = (os.getenv("LOCAL_RERANK_DEVICE") or "cpu").strip() or "cpu"
RERANK_MODEL = LOCAL_RERANK_MODEL
RERANK_ENABLED = bool(LOCAL_RERANK_MODEL)
try:
...
def _get_rerank_endpoint() -> str:
    return "local" if RERANK_ENABLED else ""


def _get_local_reranker():
    """延迟加载本地 CrossEncoder，避免服务启动时占用模型内存。"""
...
def _rerank_documents(query: str, docs: List[dict], top_k: int) -> Tuple[List[dict], Dict[str, Any]]:
    ...
    try:
        ...
        if len(reranked) != len(docs_with_rank):
            meta["rerank_error"] = "local_rerank_score_count_mismatch"
            return _sort_by_rank_score(docs_with_rank)[:top_k], meta
        return _sort_by_rank_score(reranked)[:top_k], meta
