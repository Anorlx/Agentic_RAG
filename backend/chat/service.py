...
from backend.chat.runtime import fast_model, model
from backend.chat.storage import storage
from backend.chat.supervisor import run_supervisor
...
def chat_with_agent(
    ...
    try:
        ...
        if is_hitl_resume and resume_state:
            ...
            supervisor_result = run_supervisor(effective_user_text, context_messages, ctx)
            response_content = supervisor_result.get("answer", "")
            rag_trace = normalize_rag_trace(supervisor_result.get("rag_trace"))
            rag_result = supervisor_result.get("rag_result") or {}
            resume_state_from_trace = rag_result.get("hitl_resume_state")
            next_pending_hitl = None
            if _is_hitl_trace(rag_trace):
                next_pending_hitl = _build_pending_hitl(
                    rag_trace,
...
async def chat_with_agent_stream(
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        supervisor_future = loop.run_in_executor(
            None,
            lambda: run_supervisor(effective_user_text, context_messages, ctx),
        )
        try:
            while not supervisor_future.done():
                try:
                    event = await asyncio.wait_for(output_queue.get(), timeout=0.05)
                except asyncio.TimeoutError:
                    continue
            ...
            yield f"data: {json.dumps({'type': 'error', 'content': supervisor_error})}\n\n"
        ...
        if supervisor_error is None:
            try:
                supervisor_result = await supervisor_future
            except Exception as e:
                supervisor_error = str(e)
                yield f"data: {json.dumps({'type': 'error', 'content': supervisor_error})}\n\n"

        supervisor_result = supervisor_result or {}
        full_response = supervisor_result.get("answer", "")
        rag_trace = normalize_rag_trace(supervisor_result.get("rag_trace"))
        rag_result = supervisor_result.get("rag_result") or {}
        resume_state_from_trace = rag_result.get("hitl_resume_state")
        next_pending_hitl = None
        ...
        if full_response and not next_pending_hitl:
            yield f"data: {json.dumps({'type': 'content', 'content': full_response})}\n\n"
        ...
        if next_pending_hitl:
            ...
            if is_hitl_resume and not supervisor_error:
                save_meta[PENDING_HITL_KEY] = None
            if _should_update_persistent_note(messages, persistent_note):
                try:
                    save_meta["persistent_note"] = await update_persistent_note(
